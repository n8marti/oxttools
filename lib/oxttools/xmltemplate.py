...
class Templater(object):
        ...
        for c in list(root):
            if str(c.tag).startswith(tmpl):
                ...
                if name == 'variable':
                    ...
                    if tmpl+"fallback" not in c.attrib or k not in self.vars:
                    ...
                    t = asstr(v)
                    if tmpl+"cdata" not in c.attrib or t == '':
                        root.text = t
                    else:
                        et.CDATA(t)
                elif name == 'if':
                    ...
                    node = self.process(
                        root=c,
                        context=self.xpath(c.attrib[tmpl+"path"], context, c),
                        nest=True
                    )
                    if node is None:
                        node = []
        ...
        for k, v in node.attrib.items():
            if k.startswith(tmpla):
                ...
                for t in node.attrib.keys():
                    if t.endswith(newk) and (len(t) == len(newk) or (t[-len(newk)-1] == '}' and t[:-len(newk)] != tmpla)):  # noqa: E501
    ...
    def _upscan(self, start, errorctxt, *tags):
        ...
        while res.tag not in testtags:
            if res.tag == top:
                raise SyntaxError(f"cannot find {tags[0]} above {errorctxt}")
            res = res.getparent()
        return res
        ...
        while i < len(root):
            ...
            if c.tag == self._uritag('text:hidden-text'):
                ...
                if command == 'value':
                    ...
                    res = asstr(value)
                    lines = res.strip().split("\n") if res is not None else [""]  # noqa: E501
                    c.text = lines.pop(0)
                    for line in lines:
                    ...
                    end = self._scanendfor(
                        forparent,
                        forparent.index(start),
                        var,
                        mode,
                        "endfor"
                    )
                    ...
                    for val in vals:
                        ...
                        memo = {}
                        temp = [x.__deepcopy__(memo) for x in forparent[starti:endi+1]]  # noqa: E501
                        oldvars = self.vars.copy()
                        self.vars[var] = val
                ...
                if infor is None and isinstance(parent, et._Element) and (parent.getparent() is None or root is parent or root in parent.iterancestors()):  # noqa: E501
                    return (root, i)
    ...
    def xpathall(self, path, context, base):
        try:
            ...
            raise SyntaxError(f"{e.args[0]} in xpath expression: {path}")
        return res
    ...
    def fn_replace(context, txt, regexp, repl):
        ...
        try:
            ...
            raise et.XPathEvalError(f"{e.message}: {txt=}, {regexp=}, {repl=}")
        return res

    @staticmethod
    def fn_dateformat(context, txt, *formats):
        """Converts LDML date/time format letters to LibreOffice corresponding
        codes
        """
        txt = asstr(txt)
        return txt
...
if __name__ == '__main__':
    ...
    t = Templater()
    t.define('resdir', os.path.abspath(os.path.join(os.path.dirname(__file__), "data")))  # noqa: E501

    if args.langtag is not None:
        try:
            from sldr.langtags import LangTag
        except ImportError:
            sys.path.append(os.path.join(os.path.dirname(__file__), '../../../sldr/sldr/python/lib'))  # noqa: E501
            from sldr.langtags import LangTag
